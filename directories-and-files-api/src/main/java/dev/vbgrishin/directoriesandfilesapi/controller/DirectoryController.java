...
public class DirectoryController {
    ...
    @GetMapping("/{id}")
    public DirectoryResponse findDirectoryById(@PathVariable Integer id) {
        return directoryService.findDirectoryById(id);
    }

    @GetMapping("/{id}/subdirectories")
    public List<SubdirectoryEntity> findSubDirectoriesByDirectoryId(@PathVariable Integer id) {
    ...
    public DirectoryEntity save(@RequestBody DirectoryRequest directoryRequest) {
        if (!Files.isDirectory(Paths.get(directoryRequest.path()))) {
